...
function SortSelect({ value, onChange, showCount }: { value: S2gPickerSort; onChange: (v: S2gPickerSort) => void; showCount?: boolean }) {
  return (
...
export default function S2gCataloguePickerModal({ onClose, onPick, appendToJalon = null }: Props) {
  ...
  const filteredProducts = useMemo(() => {
    const q = search.trim().toLowerCase()
    let list = isAppend ? products.filter((p) => !excludeIds.has(p.id)) : products
    list = [...list].sort((a, b) => {
  ...
  }, [products, search, isAppend, excludeIds, sort])
    ...
    >
      ...
      <div className="s2g-picker__filters">
        ...
        <SortSelect
          value={sort}
          onChange={setSort}
          showCount={step === 'qualification' || step === 'jalon'}
        />
      </div>
        ...
        <>
          ...
          <div className="s2g-picker__footer">
            <span className="text-muted s2g-picker__count">
              ...
              {filteredProducts.length > 0 ? ` · ${filteredProducts.length} visible${filteredProducts.length !== 1 ? 's' : ''}` : ''}
            </span>
