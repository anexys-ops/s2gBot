 ...
 * LaboOdmPage
 *
 * Vue des Ordres de Mission de type "labo" pour les agents du laboratoire.
...
export default function LaboOdmPage() {
  ...
  const { data: ordres = [], isLoading } = useQuery({
    queryKey: ['ordres-mission', 'labo', statutFilter],
    queryFn: () =>
  ...
  const updateMut = useMutation({
    ...
    onSuccess: () => void qc.invalidateQueries({ queryKey: ['ordres-mission', 'labo'] }),
  })
  ...
  // Filtrage côté client par BC ou site
  ...
  return (
    <ModuleEntityShell
      ...
      moduleBarLabel="Laboratoire — Ordres de Mission"
      title="Ordres de Mission — Laboratoire"
                ...
                {filtered.length === 0 && (
                  <tr>
                    <td colSpan={7} style={{ padding: '1.5rem', textAlign: 'center', color: '#6b7280' }}>
                      Aucun OdM laboratoire en attente.
                    </td>
                  </tr>
                )}
                {filtered.map((om) => {
                      ...
                      <tr key={om.id}>
                        ...
                        <td style={{ textAlign: 'center' }}>
                          <button
                            ...
                            title={isExpanded ? 'Masquer les lignes' : 'Voir les essais'}
                          >
                            {isExpanded ? '▲' : '▼'}
                          </button>
                        </td>
                        ...
                        <td>
                          ...
                          <span style={{ color: '#6b7280', fontSize: '0.8rem' }}> essai(s)</span>
                        </td>
