  ...
  return (
    <div className="card" style={{ maxWidth: '52rem' }}>
      ...
      <p style={{ color: 'var(--color-muted)', lineHeight: 1.5 }}>
        Le logo configuré ici apparaît dans l’en-tête de l’application et dans les PDF devis, factures et rapports
        d’essais. Sans fichier personnalisé, le logo S2G par défaut est utilisé. Formats acceptés : JPEG, PNG, SVG, Webp
        (3&nbsp;Mo max).
      </p>
      <div style={{ margin: '1rem 0', minHeight: 64 }}>
        <img src={logoUrl} alt={DEFAULT_APP_LOGO_ALT} style={{ maxHeight: 72, maxWidth: 280, objectFit: 'contain' }} />
      </div>
      {can ? (
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem', alignItems: 'center' }}>
          ...
          {data?.logo_is_custom ? (
            ...
            >
              {deleteMut.isPending ? 'Suppression…' : 'Retirer le logo personnalisé'}
            </button>
          ) : null}
        </div>
      ) : (
